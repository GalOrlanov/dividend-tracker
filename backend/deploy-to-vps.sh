...
echo "🌐 Application should be available at: http://129.212.145.15:5001"
echo "📊 Health check: http://129.212.145.15:5001/api/health"
EOF
...
if curl -s http://$SERVER_IP:5001/api/health > /dev/null; then
    ...
    echo "🌐 Your API is now available at: http://$SERVER_IP:5001"
    echo "📊 Health endpoint: http://$SERVER_IP:5001/api/health"
else
...
FRONTEND_CONFIG="../frontend/src/config/api.js"
...
if [ -f "$FRONTEND_CONFIG" ]; then
    ...
    # Update API URL
    sed -i.bak "s|http://10.100.102.24:5001|http://$SERVER_IP:5001|g" "$FRONTEND_CONFIG"
    sed -i.bak "s|https://your-deployed-api-url.com|http://$SERVER_IP:5001|g" "$FRONTEND_CONFIG"
    
    print_status "✅ Frontend configuration updated!"
    echo "📱 Update your frontend to use: http://$SERVER_IP:5001"
else
    print_warning "⚠️  Frontend config not found at $FRONTEND_CONFIG"
fi

echo ""
print_status "🎉 Deployment Summary:"
echo "   🌐 API URL: http://$SERVER_IP:5001"
echo "   📊 Health: http://$SERVER_IP:5001/api/health"
